...
import { mysticSkillsData } from './data/mysticSkills.js';
import { attunementData } from './data/attunements.js';
import { innerWayData } from './data/innerWays.js';
import { appState, syncInnerWaySkills } from './state.js';

// Attaching functions to window for index.html inline onClick handlers
...
const updateSkillUI = (slot) => {
    ...
    if (iconEl) iconEl.src = `assets/Icons/${getImageFileName(slotData.name)}.png`;
        ...
        return `
            <div class="weapon-ref-card ${hasWip ? 'wip-card-partial' : ''}">
                <div class="ref-card-header">
                    <img src="assets/Icons/${getImageFileName(group.name)}.png" class="weapon-icon" alt="${group.name}" onerror="this.style.display='none'">
            ...
            const videoPath = group.type === 'weapon' ?
                `assets/Weapon Previews/${group.name}/${i.action}.mp4` :
                `assets/Weapon Previews/${group.name}/${group.name}.mp4`;
            return `
...
const renderModalItems = () => {
    ...
    modalGrid.innerHTML = filtered.map(item => `
            <div class="modal-item" data-name="${item.name}" data-type="${item.type}">
                <img src="assets/Icons/${getImageFileName(item.name)}.png" class="modal-item-icon" alt="${item.name}" onerror="this.style.display='none'">
...
const getConditionalNotes = (slotData) => {
    const notes = [];
    // Check attunements
    Object.values(attunementData).flat().forEach(att => {
        if (!att.conditional || !att.modifies || !appState.attunements[att.id]) return;
        const mod = att.modifies;
        if (slotData.type === 'weapon' && mod.weapon === slotData.name && mod.action === slotData.action) {
            notes.push(att.conditional);
        }
        if (slotData.type !== 'weapon' && mod.mystic === slotData.name) {
            notes.push(att.conditional);
        }
    });
    // Check inner ways
    innerWayData.forEach(iw => {
        if (!iw.conditional || !iw.modifies || !appState.innerWays[iw.id]) return;
        const mod = iw.modifies;
        if (slotData.type === 'weapon' && mod.action === slotData.action && (!mod.weapon || mod.weapon === slotData.name)) {
            notes.push(iw.conditional);
        }
    });
    return notes;
};
...
compareBtn.addEventListener('click', () => {
    ...
    if (aWins) {
        ...
        winnerIcon.src = `assets/Icons/${getImageFileName(appState.skillA.name)}.png`;
    } else if (bWins) {
        ...
        winnerIcon.src = `assets/Icons/${getImageFileName(appState.skillB.name)}.png`;
        ...
        resultNote.textContent = 'Whoever casts/hits first wins the trade. Ping can impact this and cause variations.';
    ...
    // Append conditional attunement notes
    const condNotesA = getConditionalNotes(appState.skillA);
    const condNotesB = getConditionalNotes(appState.skillB);
    const allCondNotes = [...condNotesA, ...condNotesB];
    if (allCondNotes.length > 0) {
        const existing = resultNote.textContent;
        resultNote.textContent = (existing ? existing + ' · ' : '') + allCondNotes.join(' · ');
    }
    ...
    for (const w of weapons) {
        ...
        htmlBuffer += `
                <div class="weapon-ref-card ${anyWip ? 'wip-card-status' : ''}">
                    <div class="ref-card-header">
                        <img src="assets/Icons/${getImageFileName(w)}.png" class="weapon-icon" alt="${w}" onerror="this.style.display='none'">
                        <div class="header-text-wrap">
        ...
        for (const a of attacks) {
            ...
            st.wipD = baseSt.wipD;
            htmlBuffer += `
                    <div class="ref-attack-row ${st.isWIP ? 'wip-row' : ''}" data-preview="assets/Weapon Previews/${w}/${a}.mp4" title="Click to view full preview">
        ...
        if (st.isWIP) {
            htmlBuffer += `
                    <div class="weapon-ref-card wip-card">
                        <div class="ref-card-header">
                            <img src="assets/Icons/${getImageFileName(m)}.png" class="weapon-icon" alt="${m}" onerror="this.style.display='none'">
            ...
            htmlBuffer += `
                    <div class="weapon-ref-card ${isPartiallyWip ? 'wip-card-partial' : ''}">
                        ...
                        <div class="mystic-stat-body clickable-preview" data-preview="assets/Weapon Previews/${m}/${m}.mp4" title="Click to view full preview">
...
const renderAttunements = () => {
    ...
    for (const [slotKey, slotCfg] of Object.entries(gearSlotConfig)) {
        ...
        for (const att of items) {
            ...
            const isUninteractable = att.uninteractable;
            const weaponIcon = att.affects !== 'All Weapons' && att.affects !== 'Bow Skill'
                ? `<img src="assets/Icons/${getImageFileName(att.affects)}.png" class="att-weapon-icon" alt="${att.affects}" onerror="this.style.display='none'">`
                : '';

            html += `
                    <div class="att-card ${isUninteractable ? 'att-uninteractable' : isLocked ? 'att-locked' : ''} ${isEnabled ? 'att-enabled' : ''}" data-att-id="${att.id}">
                        <div class="att-card-top">
                            ...
                            <div class="att-card-toggle-area">
                                ${isUninteractable
                    ? '<span class="att-uninteractable-badge">ℹ️ UNINTERACTABLE</span>'
                    : isLocked
                        ...
                        <div class="att-card-footer">
                            ...
                            ${att.effectDuration ? `<span class="att-duration-badge">${att.effectDuration}</span>` : ''}
                        </div>
                    </div>
                `;
        }
...
const updateAttunementCounts = () => {
    ...
    const toggleable = allAtts.filter(a => !a.locked && !a.uninteractable);
...
const setupAttunementControls = () => {
    ...
    if (enableAllBtn) {
        enableAllBtn.addEventListener('click', () => {
            Object.values(attunementData).flat().forEach(att => {
                if (!att.locked && !att.uninteractable) appState.attunements[att.id] = true;
            });
            renderAttunements();
            refreshAttunedViews();
        });
...
const renderInnerWays = () => {
    ...
    for (const iw of innerWayData) {
        ...
        const isUninteractable = iw.uninteractable;

        html += `
                <div class="att-card iw-card ${isUninteractable ? 'att-uninteractable' : isLocked ? 'att-locked' : ''} ${isEnabled ? 'att-enabled' : ''}" data-iw-id="${iw.id}">
                    <div class="att-card-top">
                        <div class="att-card-weapon-wrap">
                ...
                : isLocked
                    ...
                    <div class="att-card-footer">
                        ...
                        ${iw.effectDuration ? `<span class="att-duration-badge">${iw.effectDuration}</span>` : ''}
                    </div>
                </div>
            `;
    }
    ...
    if (enableAll) {
        enableAll.addEventListener('click', () => {
            innerWayData.forEach(iw => {
                if (!iw.locked && !iw.uninteractable) appState.innerWays[iw.id] = true;
            });
            syncInnerWaySkills();
            renderInnerWays();
            refreshAttunedViews();
...
const updateInnerWayCounts = () => {
    const toggleable = innerWayData.filter(iw => !iw.locked && !iw.uninteractable);
        ...
        return `
                <div class="analysis-card">
                    <div class="analysis-card-header">
                        <img src="assets/Icons/${getImageFileName(res.pMove.source)}.png" class="analysis-icon">
